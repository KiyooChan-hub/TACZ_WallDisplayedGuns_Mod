...
class DryFireWarningTest {
    @Test void retriggerContinuesFromCurrentScaleAndNeverExceedsCap() {
        ...
        warning.trigger(0);
        assertEquals(1.1, warning.frames(80_000_000L).get(0).scale(), 0.0001);
        warning.trigger(80_000_000L);
        assertEquals(1.1, warning.frames(80_000_000L).get(0).scale(), 0.0001);
        assertEquals(1.3, warning.frames(240_000_000L).get(0).scale(), 0.0001);
        warning.trigger(240_000_000L);
        assertEquals(1.3, warning.frames(400_000_000L).get(0).scale(), 0.0001);
        assertEquals(1.0, warning.frames(580_000_000L).get(0).scale(), 0.0001);
    }
    ...
    @Test void retriggerDuringFadeReplacesOldPulse() {
        ...
        assertTrue(warning.frames(fading).get(0).opacity() < 1);
        warning.trigger(fading);
        var frames = warning.frames(fading);
        assertEquals(1, frames.size());
        assertEquals(1, frames.get(0).opacity(), 0.0001);
        assertEquals(1.2, warning.frames(fading + 160_000_000L).get(0).scale(), 0.0001);
    ...
    @Test void thirdClickWithinThreeSecondsArmsSoundUntilLastWarningExpires() {
        ...
        assertTrue(warning.frames(6_339_999_999L).get(0).opacity() > 0);
