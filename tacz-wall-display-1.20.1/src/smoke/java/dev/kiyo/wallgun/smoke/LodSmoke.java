...
final class LodSmoke {
    ...
    LodSmoke(){MinecraftForge.EVENT_BUS.addListener(this::tick);}
    ...
    private void tick(TickEvent.ClientTickEvent event) {
        ...
        try {
            if(!opened&&mc.screen instanceof TitleScreen&&mc.getOverlay()==null){
                ...
                mc.createWorldOpenFlows().createFreshLevel("lod-smoke-"+System.currentTimeMillis(),settings,new WorldOptions(42,false,false),r->r.registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(WorldPresets.FLAT).value().createWorldDimensions());
            }
            ...
            if(!checked){
                ...
                for(int i=0;i<scopes.length;i++){
                    ...
                    var attachment=AttachmentItemBuilder.create().setId(new ResourceLocation("tacz:"+scopes[i])).build();
                    IGun api=IGun.getIGunOrNull(gun);require(api.allowAttachment(gun,attachment),"Fixture compatibility");api.installAttachment(gun,attachment);
