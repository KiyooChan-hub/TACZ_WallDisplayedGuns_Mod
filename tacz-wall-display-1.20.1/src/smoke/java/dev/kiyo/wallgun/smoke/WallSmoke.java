...
public class WallSmoke {
    ...
    public WallSmoke() {
        ...
        MinecraftForge.EVENT_BUS.addListener((net.minecraftforge.client.event.RenderLevelStageEvent e)->{
            if(e.getStage()!=net.minecraftforge.client.event.RenderLevelStageEvent.Stage.AFTER_LEVEL)return;
    ...
    private void tick(TickEvent.ClientTickEvent event) {
        ...
        try {
                ...
                mc.getSingleplayerServer().execute(()->{
                    try {
                        ...
                        for(int i=0;i<GUNS.length;i++) {
                            ...
                            var id=new ResourceLocation(GUNS[i]);
                            ...
                            var saved=gun.saveWithFullMetadata();
                            var restored=new WallGunEntity(pos,state);restored.load(saved);
                        ...
                        for(Direction direction:Direction.values()) {
                            ...
                            var context=new net.minecraft.world.item.context.BlockPlaceContext(player,net.minecraft.world.InteractionHand.MAIN_HAND,WallGuns.decorate(sourceStack(new ResourceLocation("tacz:ak47"))),hit);
            ...
            if(phase==12) {
                ...
                mc.getSingleplayerServer().execute(()->{
                    try {
                        ...
                        mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(5.5,-55.2,5.8,180,0);
                    }catch(Throwable e){serverFailure=e;}
                });
            ...
            if(phase==6) {
                ...
                mc.getSingleplayerServer().execute(()->mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(5.5,-57.2,3.5,180,0));
                return;
            }
            if(phase==9) {
                if(ticks>=20 && ticks<400 && ticks%20==0) {
                    ...
                    mc.getSingleplayerServer().execute(()->{
                        ...
                        if(!remove)((WallGunEntity)world.getBlockEntity(pos)).setSnapshot(new GunSnapshot(sourceStack(new ResourceLocation("tacz:scar_l"))));
                    });
                }
                if(WallBatches.maxBatchGuns>4)throw new AssertionError("Wall edit rebuilt more than four guns: "+WallBatches.stats());
                if(WallBatches.lastUploadedVertices>4*GunMeshes.get(new GunSnapshot(sourceStack(new ResourceLocation("tacz:scar_l")))).vertices())throw new AssertionError("Edit upload exceeded one local cell: "+WallBatches.stats());
                if(++ticks<420)return;
                Path output=mc.gameDirectory.toPath().resolve("verification");
                ...
                mc.getSingleplayerServer().execute(()->{
                    ...
                    mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(5.5,-57.2,8,180,0);
                });
            ...
            if(phase==0) {
                ...
                mc.getSingleplayerServer().execute(()->mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(8,-57.2,8,135,0));
                phase=2;ticks=0;
            } else if(phase==2) {
                ...
                mc.getSingleplayerServer().execute(()->{
                    ...
                    for(int x=1;x<=10;x++)for(int y=-60;y<=-51;y++) {
                        ...
                        ((WallGunEntity)world.getBlockEntity(pos)).setSnapshot(new GunSnapshot(sourceStack(new ResourceLocation("tacz:scar_l"))));
                    }
                    mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(5.5,-57.2,19.5,180,0);
                });
                phase=4;ticks=0;
            } else if(phase==4) {
                ...
                mc.getSingleplayerServer().execute(()->mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(1.9,-57.1,1.3,-90,0));
                phase=10;ticks=0;
            } else if(phase==10) {
                ...
                if(phase==15) {phase=8;mc.setScreen(new Icons());}
                    ...
                    int index=phase-12;
                    mc.getSingleplayerServer().execute(()->mc.getSingleplayerServer().getPlayerList().getPlayers().get(0).connection.teleport(5.5,-55.2-index*2,5.8,180,0));
                    phase++;ticks=0;
                }
            } else if(phase==11) {
    ...
    private net.minecraft.world.item.ItemStack sourceStack(ResourceLocation id) {
        ...
        return com.tacz.guns.api.item.builder.GunItemBuilder.create().setId(id).setAmmoCount(data.getAmmoAmount()).setAmmoInBarrel(true)
                .setFireMode(data.getFireModeSet().get(0)).build();
    }
    private void verifyFrameGeometry(Path output)throws Exception {
        ...
        for(String name:new String[]{"tacz:scar_l","tacz:ak47","mk16:m4urgi10","equipped","scope/tacz:sight_exp3","scope/mk16:553_g43","scope/tacz:scope_elcan_4x"}) {
            var id=new ResourceLocation(name.startsWith("scope/")?"suffuse:n4":name.equals("equipped")?"tacz:scar_l":name);
            ...
            var r0=reference.get(0);var a0=actual.get(0);double maxError=0;
    ...
    private final class Icons extends net.minecraft.client.gui.screens.Screen {
        ...
        @Override public void render(net.minecraft.client.gui.GuiGraphics graphics,int mouseX,int mouseY,float partial) {
            ...
            for(int i=0;i<GUNS.length;i++) {
                ...
                var stack=WallGuns.decorate(sourceStack(new ResourceLocation(GUNS[i])));
