...
final class CatalogAudit {
    ...
    CatalogAudit(){MinecraftForge.EVENT_BUS.addListener(this::tick);}
    ...
    private void tick(TickEvent.ClientTickEvent e) {
        ...
        try {
            if(!opened&&mc.screen instanceof TitleScreen&&mc.getOverlay()==null) {
                ...
                mc.createWorldOpenFlows().createFreshLevel("orientation-audit-"+System.currentTimeMillis(),settings,new WorldOptions(42,false,false),r->r.registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(WorldPresets.FLAT).value().createWorldDimensions());
            }
    ...
    private final class Catalog extends Screen {
        ...
        private void load() {
            ...
            for(int n=page*SIZE;n<Math.min(ids.size(),(page+1)*SIZE);n++) {
                var id=ids.get(n);ItemStack stack=GunItemBuilder.create().setId(id).build();
            ...
            try {
                ...
                for(int i=0;i<meshes.size();i++) {
                    ...
                    for(var entry:mesh.materials().entrySet()) {
                        ...
                        for(var v:entry.getValue())buffer.vertex(pose.pose(),v.x(),v.y(),v.z()).color(v.color()).uv(v.u(),v.v()).overlayCoords(net.minecraft.client.renderer.texture.OverlayTexture.NO_OVERLAY).uv2(15728880).normal(pose.normal(),v.nx(),v.ny(),v.nz()).endVertex();
                    }
