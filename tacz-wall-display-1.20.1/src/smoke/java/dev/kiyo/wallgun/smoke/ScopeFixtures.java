...
public final class ScopeFixtures {
    ...
    public static ItemStack gun(net.minecraft.core.RegistryAccess lookup, String scope) {
        var gun=GunItemBuilder.create().setId(new ResourceLocation("suffuse:n4")).setAmmoCount(30).setAmmoInBarrel(true)
        ...
        for(String id:List.of(scope,"nmw2:muzzle_silencer_fss_covert_v_ug","suffuse:grip_td","tacz:stock_ripstock")) {
            var attachment=AttachmentItemBuilder.create().setId(new ResourceLocation(id)).build();
            IGun.getIGunOrNull(gun).installAttachment(gun,attachment);
        }
        return gun;
    }
}
