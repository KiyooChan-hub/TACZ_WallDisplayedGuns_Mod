...
- 手持木棍右键：从操作的一侧看，顺时针旋转 22.5°，16 次回到一圈。
- 手持木棍按住 Shift 右键：绕安装平面内的竖轴翻转 180°，显示枪的另一侧；两次翻面复原。检测 Shift 输入，不要求蹲姿，创造飞行可用。
...
测试源集不进入发布 JAR。各测试只使用 `run-client` 内的新存档；不编辑玩家存档。
