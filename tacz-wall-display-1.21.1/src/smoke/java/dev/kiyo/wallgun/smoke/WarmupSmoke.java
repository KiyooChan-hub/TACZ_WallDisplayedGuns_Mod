        ...
        try {
            ...
            if(!positioned) {
                ...
                mc.getSingleplayerServer().execute(()->{
                    try { var player=mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst();
                    ...
                    mc.getSingleplayerServer().execute(()->{
                        try {
                            ...
                            for(int row=0;row<10;row++)for(int col=0;col<10;col++) {
                                ...
                                stack.set(DataComponents.CUSTOM_NAME,Component.literal("New region "+row+"-"+col));
                                level.setBlock(dst,old.getBlockState(),3);((WallGunEntity)level.getBlockEntity(dst)).setSnapshot(new GunSnapshot(stack));
                            }
                            mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst().connection.teleport(256,-50.2,35,0,0);
                        }catch(Throwable ex){failure=ex;}
                    });
                }
            } else if(phase==2) {
