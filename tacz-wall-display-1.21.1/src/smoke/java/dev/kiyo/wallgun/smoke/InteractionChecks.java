...
final class InteractionChecks {
    ...
    static void run(ServerPlayer player,Path output)throws Exception {
        ...
        for(Direction face:Direction.values()) {
            ...
            var restored=new WallGunEntity(pos,state);restored.loadWithComponents(gun.saveWithFullMetadata(level.registryAccess()),level.registryAccess());
            ...
            var network=new WallGunEntity(pos,state);network.loadWithComponents(gun.getUpdateTag(level.registryAccess()),level.registryAccess());
            ...
            require(drops.size()==1&&ItemStack.matches(drops.getFirst().getItem(),original),"Exactly one intact original gun drop face="+face+" harvest="+state.canHarvestBlock(level,pos,player)+" count="+drops.size()+" actual="+drops.stream().map(d->d.getItem().save(level.registryAccess()).toString()).toList()+" expected="+original.save(level.registryAccess()));drops.forEach(ItemEntity::discard);
