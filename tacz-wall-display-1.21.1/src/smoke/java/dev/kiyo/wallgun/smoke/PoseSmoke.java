...
final class PoseSmoke {
    ...
    private int ticks,phase,faceIndex,uploads,bakes,edits,sounds;
    ...
    PoseSmoke() {
        ...
        NeoForge.EVENT_BUS.addListener((net.neoforged.neoforge.event.PlayLevelSoundEvent.AtPosition e)->{
    ...
    private void tick(ClientTickEvent.Post event) {
        ...
        try {
            if(!opened&&mc.screen instanceof TitleScreen&&mc.getOverlay()==null) {
                ...
                mc.createWorldOpenFlows().createFreshLevel("pose-smoke-"+System.currentTimeMillis(),settings,new WorldOptions(42,false,false),r->r.registryOrThrow(Registries.WORLD_PRESET).getHolderOrThrow(WorldPresets.FLAT).value().createWorldDimensions(),mc.screen);
            }
            ...
            if(!initialized) {
                ...
                if(toolSmoke) {
                    ...
                    WallGuns.ITEM.get().appendHoverText(new ItemStack(WallGuns.ITEM.get()),Item.TooltipContext.EMPTY,tooltip,TooltipFlag.NORMAL);
                ...
                server(()->{
                    ...
                    try {InteractionChecks.run(mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst(),out());}catch(Exception ex){throw new RuntimeException(ex);}
                    sounds=0;
                    ...
                    var player=mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst();player.getAbilities().flying=true;player.onUpdateAbilities();player.connection.teleport(5.5,-57.2,19.5,180,0);
            ...
            if(phase==0) {
                ...
                require(WallBatches.lastUploadedVertices<=4*13468,"Pose edit rebuilt beyond local cell: "+WallBatches.stats());
                if(++ticks<380)return;
                ...
                if(toolSmoke && ticks==10)server(()->mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst().setItemInHand(InteractionHand.MAIN_HAND,new ItemStack(WallGunConfig.adjustmentItemId().toString().equals("minecraft:stick")?Items.FEATHER:Items.STICK)));
    ...
    private void prepareFace() {
        ...
        server(()->{
            ...
            var player=mc.getSingleplayerServer().getPlayerList().getPlayers().getFirst();player.setItemInHand(InteractionHand.MAIN_HAND,toolSmoke?configuredTool():new ItemStack(Items.STICK));player.setItemInHand(InteractionHand.OFF_HAND,new ItemStack(Items.STONE));
